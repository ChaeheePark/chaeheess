...
우리는 가상 에이전트를 장면 중앙의 의자에 완전히 초점을 맞추고 에이전트가 적절한 신체 제스처를 사용하여 한 줄씩 내레이션하는 스토리를 선택하거나, 에이전트가 텍스트 및 연관된 신체 제스처를 사용하여 응답하는 대화의 일부로 텍스트 줄을 보낼 수있음
...
Transformer로 Encoded feature을 만들고 그 feature에 해당하는 gesture을 mapping해주는것을 활용 

이때 이전 gesture 정보까지 담음 -> 우리 프로젝트에 활용해볼점 **
